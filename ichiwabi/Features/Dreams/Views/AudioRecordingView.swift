...
struct AudioRecordingView: View {
    ...
    @State private var isGenerating = false {
        didSet {
            print("🔘 isGenerating changed to: \(isGenerating)")
        }
    }
                    ...
                    VStack(spacing: 16) {
                        if let url = recordedAudioURL {
                            ...
                            if let style = selectedStyle {
                                Button {
                                    print("🔘 Generate button tapped - BEFORE STATE CHANGE")
                                    print("🔘 Current isGenerating state: \(isGenerating)")
                                    Task { @MainActor in
                                        isGenerating = true
                                        print("🔘 Generate button tapped - AFTER STATE CHANGE")
                                        print("🔘 New isGenerating state: \(isGenerating)")
    ...
    private func generateDream(url: URL, style: DreamVideoStyle) {
        print("\n🎯 ==================== GENERATE DREAM START ====================")
        print("🎯 Button tapped at: \(Date())")
        print("🎯 URL: \(url)")
        print("🎯 Style: \(style)")
        print("🎯 Current isGenerating state: \(isGenerating)")
        print("🎯 Current thread: \(Thread.current.isMainThread ? "Main" : "Background")")
        ...
        Task {
            do {
                ...
                if authService.currentUserId == nil {
                    print("🎯 No user ID, signing in anonymously")
                    try await authService.signInAnonymously()
                }
                
                guard let userId = authService.currentUserId else {
                    print("🎯 Failed to get user ID after sign in")
                ...
                print("🎯 Using title: \(dreamTitle)")
                
                // Process and upload the video
                print("🎯 About to call processAndUploadVideo")
                ...
                print("🎯 processAndUploadVideo completed")
                print("🎯 Results:")
                print("🎯 - Video URL: \(processedResult.videoURL)")
                print("🎯 - Audio URL: \(processedResult.audioURL)")
                print("🎯 - Local path: \(processedResult.localPath)")
                ...
                if let underlying = nsError.userInfo[NSUnderlyingErrorKey] as? NSError {
                    ...
                    print("🎯   - Description: \(underlying.localizedDescription)")
                }
                
                await MainActor.run {
                    print("🎯 Updating UI state after error")
                    self.isGenerating = false
                    self.showError = true
                    audioService.errorMessage = "Failed to generate dream: \(error.localizedDescription)"
                    print("🎯 UI state updated after error")
                }
            }
        }
    }
